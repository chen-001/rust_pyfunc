...
fn main() {
    ...
    // B 侧 = 全市场（barra_all）
    let bpath = format!("barra_all_{date}.txt");
    let fb = std::fs::File::open(&bpath).unwrap_or_else(|_| panic!("缺少 barra_all 文件 {bpath}"));
    let mut bcodes: Vec<String> = Vec::new();
    let mut binds: Vec<u32> = Vec::new();
    for line in BufReader::new(fb).lines() {
        let line = line.unwrap();
        let mut it = line.split_whitespace();
        let c = it.next().unwrap().to_string();
        let i: u32 = it.next().unwrap().parse().unwrap();
    ...
    let n = codes.len();
    let nb = bcodes.len();
    eprintln!("date={date} universe={n} b_all={nb}");

    let each = |c: &String| -> Option<[EvStream; N_EV]> {
    ...
    let mut all_codes: Vec<String> = codes.clone();
    all_codes.extend(bcodes.iter().filter(|c| !codes.contains(c)).cloned());
    let streams: Vec<Option<[EvStream; N_EV]>> = all_codes.par_iter().map(each).collect();
    // 索引映射：B 在 all_codes 中的位置
    let mut bpos: Vec<usize> = Vec::with_capacity(nb);
    {
        use std::collections::HashMap;
        let mut map: HashMap<&str, usize> = HashMap::new();
        for (k, c) in all_codes.iter().enumerate() {
            map.insert(c.as_str(), k);
        }
        for c in &bcodes {
            bpos.push(*map.get(c.as_str()).unwrap());
        }
    }
    let n_all = all_codes.len();
    eprintln!("n_all={n_all}");
    ...
    let mut slices: Vec<Option<Vec<Vec<i64>>>> = Vec::with_capacity(n_all);
    for s in &streams {
    ...
    for ev in 0..N_EV {
        let mut mat = vec![[f32::NAN; M]; n * nb];
        let si: Vec<Option<&Vec<i64>>> = slices.iter().map(|s| s.as_ref().map(|se| &se[ev])).collect();
        let idxs: Vec<(usize, usize)> = (0..n).flat_map(|i| bpos.iter().map(move |&j| (i, j))).collect();
        ...
        for (ij, p_ab, p_ba) in res {
            ...
            mat[i * nb + j] = m_ab.map(|x| x as f32);
        }
        all_pm.push(mat);
    }
    ...
    for ev in 0..N_EV {
        let mat = &all_pm[ev];
        // 跨对百分位：rhit_score 与 comp（A×B 全部对）
        ...
        // strength 矩阵
        let mut strength = vec![[f64::NAN; 2]; n * nb]; // [0]=strength, [1]=rhit_score
        for i in 0..n {
            for j in 0..nb {
                let m = mat[i * nb + j];
                let rs = if m[0].is_finite() && m[1].is_finite() {
                ...
                strength[i * nb + j][0] = st;
                strength[i * nb + j][1] = rs;
            }
        }
        // marg（A 侧只对全市场平均；B 侧无 marg，特异性只用 A 的基线——旧 agent 建议的
        // "净耦合减该股对全市场平均耦合" 在 A 侧完全实现）
        let mut row_valid = vec![0usize; n];
        let mut row_sum = vec![0.0f64; n];
        for i in 0..n {
            for j in 0..nb {
                if strength[i * nb + j][0].is_finite() {
                    row_valid[i] += 1;
                    row_sum[i] += strength[i * nb + j][0];
                }
            }
        }
        let marg: Vec<f64> = (0..n)
            ...
            .collect();
        let mut spec = vec![f64::NAN; n * nb];
        for i in 0..n {
            for j in 0..nb {
                let st = strength[i * nb + j][0];
                if st.is_finite() && marg[i].is_finite() {
                    let sp = st - marg[i];
                    spec[i * nb + j] = sp;
                }
            }
        ...
        for &(i, j, _st, sp) in cands.iter().take(200) {
            ...
            top_pairs.push(TopPair {
                event: ev,
                a: codes[i].clone(),
                b: all_codes[bpos[j]].clone(),
                ia: i,
                ib: j,
                strength: strength[i * nb + j][0] as f32,
                spec: sp as f32,
                rhit_f: m[0],
                rhit_b: m[1],
                rmed_f: m[2],
