"""同热点股票池「行业拓展」IC 评估
输入: sandbox_hot_industry/out_{date}.json (5 天)
输出: results_ic.json / 汇总表打印
...
DATES = [20240104, 20240603, 20241008, 20260105, 20260710, 20260717]
OUTDIR = "/home/chenzongwei/rust_pyfunc/sandbox_hot_industry"

# ============ 收益口径（用户指定，2026-08 更正） ============
# read_daily(ret=1) 第 D 行 = 当日收益 close[D]/close[D-1]-1（trailing，D 收盘可知），不是前瞻！
# 前瞻收益必须用复权收盘价位移:
close = read_daily(close=1)
close.index = pd.to_datetime(close.index)
_retD = close / close.shift(1) - 1       # 第 D 行 = D 当日收益（D-1→D），因子可用信息
gap1 = close.shift(-1) / close - 1        # 第 D 行 = D→D+1 前瞻1日收益（目标）
gap5 = close.shift(-5) / close - 1        # 第 D 行 = D→D+5 前瞻5日收益（目标）
r1 = read_daily(ret=1)                    # 仅留作一致性校验（r1.loc[D] == ret_D.loc[D]）
...
def build_day(date):
    d = json.load(open(f"{OUTDIR}/out_{date}.json"))
    rows = d["stocks"]
    df = pd.DataFrame(
        {
    ...
    # 因子端: D 当日收益（D 收盘可知）；目标端: gap1/gap5 的 D 行（D→D+1 / D→D+5 前瞻）
    ret_D = _retD.loc[dt]
    ret_N = gap1.loc[dt]
    ret_N5 = gap5.loc[dt]
    # 一致性校验（assert 等价：目标 gap1(D) 与 r1.loc[D+1] 逐值相同；末日 gap1 全 NaN）
    nx = next_day(dt)
    if dt < r1.index[-1]:
        assert np.nanmax(np.abs(r1.loc[nx].values - ret_N.values)) < 1e-12, f"gap1对齐不一致 {date}"
    else:
        assert ret_N.isna().all(), f"末日 gap1 应为全 NaN {date}"
...
with open(f"{OUTDIR}/results_ic.json", "w") as f:
