    ...
    *counter = counter.wrapping_add(1);
    ...
    for k in 0..6 {
        if lag_n[k] > 0 {
            ...
            if r > peak_r {
                peak_r = r;
                peak_lag = [0.1f32, 0.25, 0.5, 1.0, 2.0, 5.0][k];
            }
        }
    }
    out[11] = peak_r;
    ...
    out[15] = ent;
    out[16] = hhi;
    out[17] = if first_lag_us > 0 {
        first_lag_us as f32 / 1e6
    } else {
        -1.0
    };
    out[18] = if umass > EPS {
    ...
    out[19] = breadth;
    out[20] = stock_hhi;
    out[21] = if breadth > 1.0 {
        entropy_raw / breadth.ln()
    } else {
        0.0
    };
    out[22] = entropy_raw.exp();
    out[23] = if mstock_tot > EPS {
        top1 / mstock_tot
    } else {
        0.0
    };
    out[24] = if umass > EPS {
        (m_dt_sum as f32) / umass
    } else {
        0.0
    };
    let mean_t = out[24] as f64;
    out[25] = if umass > EPS {
        (m_dt2_sum / umass as f64 - mean_t * mean_t).max(0.0).sqrt() as f32
    } else {
        0.0
    };
    out[26] = n_flips as f32;
    out[27] = if first_same_us > 0 {
        first_same_us as f32 / 1e6
    } else {
        -1.0
    };
    out[28] = if first_opp_us > 0 {
        first_opp_us as f32 / 1e6
    } else {
        -1.0
    };
    out[29] = if t10_us > 0 {
    ...
    out[30] = if urg_vol > EPS {
        same_vol as f32 / urg_vol
    } else {
        0.0
    };
    ...
    out[34] = if p0d > 1e-9 {
        (n_urgf as f64 / (ntr + 1e-9) / p0d) as f32
    } else {
        0.0
    };
    out
...
fn stock_version_factors(
    ...
    for si in 0..n {
        ...
        let ret_pre = if spx > 0.0 {
            (vwap_pre - spx) / spx
        } else {
            0.0
        };
        let ret_post = if spx > 0.0 {
            (vwap_post - spx) / spx
        } else {
            0.0
        };
    ...
    let arr =
        Array2::from_shape_vec((n, NCOLS), flat).unwrap_or_else(|_| Array2::zeros((0, NCOLS)));
...
fn main() {
    ...
    let n = time_us.len();
    eprintln!(
        "read {n_stocks} stocks, {:.1}M trades in {:?}",
        n as f64 / 1e6,
        t0.elapsed()
    );
    ...
    eprintln!(
        "q95={q95:+.5} q5={q5:+.5} q95_abs={q95_abs:.5} in {:?}",
        t1.elapsed()
    );
    ...
    for i in 0..n {
        pref_vol[i + 1] = pref_vol[i] + vol_s[i] as f64;
    }
    // per-stock 在 time_s 中的位置索引（有序），供价格冲击查种子股窗口价格
    let mut stock_pos: Vec<Vec<u32>> = vec![Vec::new(); n_stocks];
    for i in 0..n {
        stock_pos[cid_s[i] as usize].push(i as u32);
    }
    // per-stock price·vol / vol 前缀和（价格冲击 O(1) 查询，避免遍历该股窗口的随机访问）
    let stock_cum_pv: Vec<Vec<f64>> = (0..n_stocks)
        .into_par_iter()
        .map(|c| {
            let mut acc = 0.0;
            let mut v = Vec::with_capacity(stock_pos[c].len() + 1);
            v.push(0.0);
            for &p in &stock_pos[c] {
                acc += price_s[p as usize] as f64 * vol_s[p as usize] as f64;
                v.push(acc);
            }
            v
        })
        .collect();
    let stock_cum_v: Vec<Vec<f64>> = (0..n_stocks)
        .into_par_iter()
        .map(|c| {
            let mut acc = 0.0;
            let mut v = Vec::with_capacity(stock_pos[c].len() + 1);
    ...
    let t3 = Instant::now();
    let mut sell_by_stock: Vec<(Vec<u32>, Vec<i8>, Vec<u32>)> =
        vec![(Vec::new(), Vec::new(), Vec::new()); n_stocks];
    ...
    for i in 0..n {
        ...
        let d = dir_s[i];
        if r > q95 {
            sell_by_stock[c].0.push(i as u32);
            sell_by_stock[c].1.push(d);
            sell_by_stock[c].2.push(c as u32);
        }
        if r < q5 {
            buy_by_stock[c].0.push(i as u32);
            buy_by_stock[c].1.push(d);
            buy_by_stock[c].2.push(c as u32);
        }
        if ab > q95_abs {
            both_by_stock[c].0.push(i as u32);
            both_by_stock[c].1.push(d);
            both_by_stock[c].2.push(c as u32);
            both_time.push(time_s[i]);
            both_abratio.push(ab);
            both_vol.push(vol_s[i]);
            both_dir.push(d);
            both_cid.push(c as u32);
        }
    }
    eprintln!(
        "urg grouped in {:?}: sell/buy/both = {}/{}/{}",
        t3.elapsed(),
    ...
    // 验证打印
    {
        ...
        if !s.is_empty() {
            ...
            let ret = if seed_px > 0.0 {
                (vwap_post - seed_px as f64) / seed_px as f64
            } else {
                0.0
            };
    ...
    let probe_vals = stock_version_factors(
        &sell_by_stock[tid].0,
        &sell_by_stock[tid].1,
        &sell_by_stock[tid].2,
        &time_s,
    ...
    eprintln!(
        "feat_len = {feat_len} (NCOLS={NCOLS}), OUT_LEN = {}",
        feat_len * 3
    );
    ...
    let valid = by_stock_out
        .iter()
        .filter(|arr| {
            arr.iter().any(|o| {
                o.as_ref()
                    .map_or(false, |v| v.iter().any(|x| x.is_finite()))
            })
        })
        .count();
    eprintln!(
        "有效股票 {valid}, 总耗时 {:?}, vals_len={}",
        t0.elapsed(),
        valid * feat_len * 3
    );
}
