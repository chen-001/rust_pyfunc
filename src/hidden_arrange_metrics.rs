...
fn parse_chunk(data: &[u8], with_retreat: bool, adj: bool) -> Vec<TradeRecord> {
    ...
    for i in 0..data.len() {
        if data[i] == b'\n' {
            ...
            if let Some(r) = parse_line(line, with_retreat, adj) {
                out.push(r);
            }
            start = i + 1;
        }
...
fn parse_market_line(line: &[u8], adj: bool) -> Option<MarketRecord> {
    ...
    let mut ask_prcs = [0f64; 10];
    let mut ask_vols = [0f64; 10];
    let mut bid_prcs = [0f64; 10];
    let mut bid_vols = [0f64; 10];
    for i in 0..10 {
...
fn parse_market_chunk(data: &[u8], adj: bool) -> Vec<MarketRecord> {
    ...
    for i in 0..data.len() {
        if data[i] == b'\n' {
            ...
            if let Some(r) = parse_market_line(line, adj) {
                out.push(r);
            }
            start = i + 1;
        }
...
impl Window {
    fn load(path: &str) -> std::io::Result<Self> {
        ...
        for i in 0..n {
            volume[i] = rd_i64(&buf, &mut p);
        }
        let mut price = vec![0f64; n];
        for i in 0..n {
            price[i] = rd_f64(&buf, &mut p);
        }
        let mut flag = vec![0i32; n];
        for i in 0..n {
            flag[i] = rd_i32(&buf, &mut p);
        }
        let mut bid_order = vec![0i64; n];
        for i in 0..n {
            bid_order[i] = rd_i64(&buf, &mut p);
        }
        let mut ask_order = vec![0i64; n];
        for i in 0..n {
            ask_order[i] = rd_i64(&buf, &mut p);
        }
        let mut time_sec = vec![0f64; n];
        for i in 0..n {
            time_sec[i] = rd_f64(&buf, &mut p);
        }
        let mut active_side = vec![0i8; n];
        for i in 0..n {
            active_side[i] = i8::from_le_bytes([buf[p]]);
            p += 1;
        }
        let mut active_order = vec![0i64; n];
        for i in 0..n {
            active_order[i] = rd_i64(&buf, &mut p);
        }
        let mut passive_order = vec![0i64; n];
        for i in 0..n {
            passive_order[i] = rd_i64(&buf, &mut p);
        }
        // 特征矩阵
...
fn axis_order(win: &Window, axis: &str) -> Vec<usize> {
    ...
    match axis {
        ...
        "ACTIVE_ORDER" => {
            idx.sort_by(|&a, &b| {
                let ka = if win.active_order[a] >= 0 {
                    win.active_order[a]
                } else {
                    i64::MAX
                };
                let kb = if win.active_order[b] >= 0 {
                    win.active_order[b]
                } else {
                    i64::MAX
                };
        ...
        "PASSIVE_ORDER" => {
            idx.sort_by(|&a, &b| {
                let ka = if win.passive_order[a] >= 0 {
                    win.passive_order[a]
                } else {
                    i64::MAX
                };
                let kb = if win.passive_order[b] >= 0 {
                    win.passive_order[b]
                } else {
                    i64::MAX
                };
...
fn run_stage1_branch(
    ...
    while !cur_level.is_empty() {
        ...
        if !cur_level_ds.is_empty() {
            level_ds.push(cur_level_ds);
        }
...
impl Stage1Result {
    pub fn to_json(&self) -> String {
        let mut s = format!(
            "{{\"rho0\":{:.4},\"rho1\":{:.4},\"best_name\":\"{}\",\"branches\":{{",
...
pub fn run_stage1(win: &Window, y: &[f64]) -> Stage1Result {
    ...
    for b in &branches {
        if b.rho_final > best.2 {
            best = (format!("{}_{}", b.axis, b.depth), b.g.clone(), b.rho_final);
        }
    }
    for (i, beta) in [1, 2, 5, 10].iter().enumerate() {
        if votes[i].1 > best.2 {
            best = (format!("VOTE_B{}", beta), votes[i].0.clone(), votes[i].1);
        }
    }
    ...
    Stage1Result {
        ...
        g: best.1.clone(),
        rho: best.2,
        best_name: best.0.clone(),
        branches,
        votes,
        path_rhos: vec![rho0, rho_desc, rho_active, rho_random],
    }
}
...
fn is_discrete(t: &str) -> bool {
    matches!(
        t,
        "BIN8" | "BIN16" | "BIN32" | "TOP20" | "TOP10" | "TOP05" | "TOP02" | "TOP01"
    )
}
...
fn run_full_game(
    ...
    y: &[f64],
    target: &str,
    graphs: &std::collections::HashMap<&'static str, EdgeSet>,
) -> (Stage1Result, Stage2Result, Option<Stage3Result>) {
    let t0 = std::time::Instant::now();
    let s1 = run_stage1(win, y);
    let t1 = std::time::Instant::now();
    let s2 = run_stage2_with_graphs(&s1.g, s1.rho, y, graphs);
    let t2 = std::time::Instant::now();
    let s3 = if is_discrete(target) {
        Some(run_stage3(&s2.g, y, &[0.5], 32))
    } else {
        None
    };
    (s1, s2, s3)
...
fn compute_all_factors(code: &str, date: i64, tws: &[f64], n_null: usize) {
    ...
    for seg_id in 1..=4u32 {
        ...
        for &tw in tws {
            ...
            let windows: Vec<(f64, Window)> = all_wins
                .last()
            ...
            for (ts, win) in &windows {
                ...
                for target in TARGETS_C.iter() {
                    ...
                    for (name, rv) in &real_f {
                        let null_vals: Vec<f64> = null_maps
                            .iter()
                            .map(|m| *m.get(name).unwrap_or(&f64::NAN))
                            .filter(|v| !v.is_nan())
                            .collect();
                        let mean_n = if null_vals.is_empty() {
                            f64::NAN
                        } else {
                            null_vals.iter().sum::<f64>() / null_vals.len() as f64
                        };
                        let std_n = if null_vals.len() > 1 {
                            (null_vals.iter().map(|v| (v - mean_n).powi(2)).sum::<f64>()
                                / null_vals.len() as f64)
                                .sqrt()
                        } else if null_vals.len() == 1 {
                            0.0
                        } else {
                            f64::NAN
                        };
                        let excess = if mean_n.is_nan() {
                            f64::NAN
                        } else {
                            rv - mean_n
                        };
                        let null_z = if mean_n.is_nan() || std_n.is_nan() {
                            f64::NAN
                        } else {
                            (rv - mean_n) / (std_n + 1e-8)
                        };
                        total_cols += 3;
                        if samples.len() < 8 {
                            samples.push(format!(
                                "SEG{}_{}_{}_{}_RAW/EXCESS/NULL_Z@{:.0} = {:.4}/{:.4}/{:.4}",
            ...
            println!(
                ...
                seg_id,
                tw_label,
                windows.first().map(|(_, w)| w.n).unwrap_or(0)
            );
        }
    }
    println!("\n=== 折中方案C 因子统计 ===");
    ...
    println!(
        "单时间戳因子列(含RAW/EXCESS/NULL_Z): {}",
        total_cols / win_count.max(1)
    );
...
pub fn compute_hidden_arrange_full(
    code: &str,
    date: i64,
    ...
    let out_vals: Vec<f32> = tmpl
        .iter()
        .map(|n| map.get(n.as_str()).copied().unwrap_or(f32::NAN))
        .collect();
    Ok((tmpl, out_vals))
}
...
fn sameprice_runlen(price: &[f64]) -> Vec<f64> {
    ...
    for &gg in &grp {
        counts[gg as usize] += 1.0;
    }
    for i in 0..n {
        out[i] = counts[grp[i] as usize];
    }
    out
}
...
pub fn align_and_build(trade: &[TradeRecord], market: &[MarketRecord]) -> Window {
    ...
    order.sort_by(|&a, &b| {
        trade[a]
            .time_us
            .cmp(&trade[b].time_us)
            .then(trade[b].index.cmp(&trade[a].index))
    });
    ...
    for i in 0..n {
        ...
        let (b5, a5) = (0..5).fold((0f64, 0f64), |(b, a), j| {
            (b + m.bid_vols[j] as f64, a + m.ask_vols[j] as f64)
        });
        bvol5[i] = b5;
        avol5[i] = a5;
        let (b10, a10) = (0..10).fold((0f64, 0f64), |(b, a), j| {
            (b + m.bid_vols[j] as f64, a + m.ask_vols[j] as f64)
        });
        bvol10[i] = b10;
        avol10[i] = a10;
    ...
    let imb1: Vec<f64> = (0..n)
        .map(|i| safe_div(bvol1[i] - avol1[i], bvol1[i] + avol1[i]))
        .collect();
    let imb5: Vec<f64> = (0..n)
        .map(|i| safe_div(bvol5[i] - avol5[i], bvol5[i] + avol5[i]))
        .collect();
    let imb10: Vec<f64> = (0..n)
        .map(|i| safe_div(bvol10[i] - avol10[i], bvol10[i] + avol10[i]))
        .collect();
    let microprice: Vec<f64> = (0..n)
        .map(|i| safe_div(bid1[i] * avol1[i] + ask1[i] * bvol1[i], bvol1[i] + avol1[i]))
        .collect();
    ...
    for i in 1..n {
        q_bid1_chg[i] = bvol1[i] - bvol1[i - 1];
        q_ask1_chg[i] = avol1[i] - avol1[i - 1];
    }
    let q_bid1_consume: Vec<f64> = (0..n)
        .map(|i| {
            let prev = if i > 0 { bvol1[i - 1] } else { bvol1[i] };
            safe_div((prev - bvol1[i]).max(0.0), prev)
        })
        .collect();
    let q_ask1_consume: Vec<f64> = (0..n)
        .map(|i| {
    ...
    for i in 0..n {
        book_feats.push(spread[i]);
        book_feats.push(imb1[i]);
        book_feats.push(imb5[i]);
        book_feats.push(imb10[i]);
        book_feats.push(bvol5[i]);
        book_feats.push(avol5[i]);
        book_feats.push(micro_off[i]);
        book_feats.push(slope_bid[i]);
        book_feats.push(slope_ask[i]);
        book_feats.push(signed_imb10[i]);
        book_feats.push(signed_imb50[i]);
        book_feats.push(dt_prev[i]);
        book_feats.push(mid_ret10[i]);
        book_feats.push(mid_vol50[i]);
    }
    let mut queue_feats = Vec::with_capacity(n * 10);
    for i in 0..n {
        queue_feats.push(bvol1[i]);
        queue_feats.push(avol1[i]);
        queue_feats.push(q_bid1_chg[i]);
        queue_feats.push(q_ask1_chg[i]);
        queue_feats.push(q_bid1_consume[i]);
        queue_feats.push(q_ask1_consume[i]);
        queue_feats.push(imb1[i]);
        queue_feats.push(spread[i]);
        queue_feats.push(side[i] as f64);
        queue_feats.push(q_sameprice[i]);
    }
    let mut arrival_feats = Vec::with_capacity(n * 6);
    for i in 0..n {
        arrival_feats.push(dt_prev[i]);
        arrival_feats.push(a_dt_mean8[i]);
        arrival_feats.push(a_dt_mean32[i]);
        arrival_feats.push(a_dt_cv32[i]);
        arrival_feats.push(a_cnt1s[i]);
        arrival_feats.push(a_cnt5s[i]);
    }
    ...
    for i in 0..win.n {
        ...
        match seg {
            ...
            4 => {
                ...
                if is_buy && ts >= SEG_LATE30_START && ts <= SEG_LATE30_END {
                    idx.push(i);
                }
            }
            _ => {}
...
pub fn slide_time_windows(
    ...
    while start + tw <= t1 + 1e-6 {
        ...
        let mut idx = vec![];
        for i in 0..seg.n {
            if seg.time_sec[i] >= start && seg.time_sec[i] < end {
                idx.push(i);
            } else if seg.time_sec[i] >= end {
                break;
            }
        }
        if idx.len() >= n_min {
...
pub fn slice_window(win: &Window, idx: &[usize]) -> Window {
    ...
    let get_f = |arr: &[f64], nf: usize| -> Vec<f64> {
        ...
        for &i in idx {
            out.extend_from_slice(&arr[i * nf..(i + 1) * nf]);
        }
        out
    };
    Window {
...
fn knn_graph(win: &Window, feats: &[f64], nfeat: usize, k: usize, out: &mut EdgeSet) {
    ...
    for i in 0..n {
        ...
        let mut d2: Vec<(f64, usize)> = (0..n)
            .map(|j| {
                let dot: f64 = (0..nfeat)
                    .map(|jj| z[i * nfeat + jj] * z[j * nfeat + jj])
                    .sum();
...
pub fn build_graphs(win: &Window) -> HashMap<&'static str, EdgeSet> {
    ...
    for &(col_fn, name) in &[
        (
            (|w: &Window, i: usize| w.active_order[i]) as fn(&Window, usize) -> i64,
            "ACTIVE_EXACT",
        ),
        (
            (|w: &Window, i: usize| w.passive_order[i]) as fn(&Window, usize) -> i64,
            "PASSIVE_EXACT",
        ),
    ] {
        ...
        for members in groups.values() {
            for a in 0..members.len() {
                for b in (a + 1)..members.len() {
                    add_edge(e, members[a], members[b]);
                }
            }
        }
    }
    ...
    for members in by_price.values() {
        for a in 0..members.len() {
            for b in (a + 1)..members.len() {
                add_edge(e0, members[a], members[b]);
            }
        }
    }
    // PRICE_T2: 按价格排序滑窗
    ...
    for a in 0..n {
        for b in (a + 1)..n {
            if sp[b] - sp[a] > 2.0 * PRICE_TICK + 1e-9 {
                break;
            }
    ...
    for i in 0..n {
        by_side.entry(win.active_side[i]).or_default().push(i);
    }
    let es = graphs.get_mut("SIDE_EXACT").unwrap();
    for members in by_side.values() {
        for a in 0..members.len() {
            for b in (a + 1)..members.len() {
                add_edge(es, members[a], members[b]);
            }
        }
    }
    // KNN
    knn_graph(
        ...
        14,
        10,
        graphs.get_mut("BOOK_K10").unwrap(),
    );
    knn_graph(
        win,
        &win.book_feats,
        14,
        30,
        graphs.get_mut("BOOK_K30").unwrap(),
    );
    knn_graph(
        win,
        &win.queue_feats,
        10,
        10,
        graphs.get_mut("QUEUE_K10").unwrap(),
    );
    knn_graph(
        win,
        &win.arrival_feats,
        6,
        10,
        graphs.get_mut("ARRIVAL_K10").unwrap(),
    );
    graphs
}
...
pub fn graph_bisect(
    ...
    comps.sort_by(|a, b| {
        b.len()
            .cmp(&a.len())
            .then(a.iter().min().cmp(&b.iter().min()))
    });
    ...
    for comp in comps {
        if (comp.len() as f64) > half {
            ...
            if a.len() <= b.len() {
                a.extend(comp.iter().copied());
            } else {
                b.extend(comp.iter().copied());
            }
        }
    }
    balance(a, b)
...
fn graph_candidate(g: &[f64], a: &[usize], b: &[usize], high_a: bool) -> Vec<f64> {
    ...
    for (k, &i) in ao.iter().enumerate() {
        g2[i] = a_vals[k];
    }
    let mut bo = b.to_vec();
    bo.sort_by(|&x, &y| g[x].partial_cmp(&g[y]).unwrap());
    for (k, &i) in bo.iter().enumerate() {
        g2[i] = b_vals[k];
    }
    g2
}
...
fn run_branch(
    ...
    let blocks = if scope == "GLOBAL_D4" {
        vec![(0..n).collect::<Vec<_>>()]
    } else {
        split_blocks(n, n_blocks)
    };
    let mut g = g1.to_vec();
    let mut rho_cur = bank.query(&g);
    let rho1 = rho_cur;
    ...
    let rho_final = bank.query(&g);
    let r1 = average_rank(g1);
    let r2 = average_rank(&g);
...
impl Stage2Result {
    pub fn to_json(&self) -> String {
        let mut s = format!(
            "{{\"rho2\":{:.4},\"s2_best_name\":\"{}\",\"s2_branches\":{{",
            self.rho, self.best_name
        );
        for (i, b) in self.branches.iter().enumerate() {
            if i > 0 {
                s.push(',');
...
pub fn build_adj_lists(
    graphs: &HashMap<&'static str, EdgeSet>,
    n: usize,
) -> HashMap<&'static str, Vec<Vec<usize>>> {
...
pub fn run_stage2_with_adjs(
    g1: &[f64],
    rho1: f64,
    y: &[f64],
    adjs: &HashMap<&'static str, Vec<Vec<usize>>>,
) -> Stage2Result {
    let bank = Bank::new(y.to_vec());
    let n = y.len();
    ...
    let mut pos_idx: Vec<usize> = gains
        .iter()
        .enumerate()
        .filter(|(_, &g)| g > 0.0)
        .map(|(i, _)| i)
    ...
    let owned_pool: Vec<S2Branch> = pool
        .iter()
        .map(|&b| S2Branch {
            graph: b.graph.clone(),
            scope: b.scope.clone(),
            g: b.g.clone(),
            rho1: b.rho1,
            rho_final: b.rho_final,
            trajectory: b.trajectory.clone(),
            node_ds: b.node_ds.clone(),
            accepts: b.accepts.clone(),
            n_queries: b.n_queries,
            path_distance: b.path_distance,
        })
        .collect();
    let votes = vec![
    ...
    for b in &branches[1..] {
        if b.rho_final > best_single.rho_final {
            best_single = b;
        }
    }
    ...
    if best_single.rho_final > best.2 {
        best = (
            format!("{}_{}", best_single.graph, best_single.scope),
            best_single.g.clone(),
            best_single.rho_final,
        );
    }
    for (i, beta) in [1, 2, 5, 10].iter().enumerate() {
        if votes[i].1 > best.2 {
...
fn update_posterior(
    ...
    if s > 0.0 {
        for i in 0..n {
            for j in 0..m {
                target[i * m + j] = (1.0 - lr_eff) * p[i * m + j];
            }
...
fn posterior_stats(
    ...
    let top1: Vec<f64> = (0..n)
        .map(|i| (0..m).map(|j| p[i * m + j]).fold(0f64, |a, b| a.max(b)))
        .collect();
    let post_mean: Vec<f64> = (0..n)
        .map(|i| (0..m).map(|j| j as f64 * p[i * m + j]).sum())
        .collect();
    let post_var: Vec<f64> = (0..n)
        .map(|i| {
            let mean = post_mean[i];
            (0..m)
                .map(|j| (j as f64).powi(2) * p[i * m + j])
                .sum::<f64>()
                - mean * mean
        })
        .collect();
    ...
    fn tail_conf(p: &[f64], n: usize, m: usize, s: &[usize]) -> f64 {
        let mut tot = 0.0;
        for i in 0..n {
            tot += s.iter().map(|&j| p[i * m + j]).sum::<f64>();
        }
        tot / n as f64
    }
    ...
    vec![
        ...
        tail_entropy(p, n, m, &s20),
        tail_entropy(p, n, m, &s10),
        tail_entropy(p, n, m, &s05),
        tail_entropy(p, n, m, &s01),
        tail_conf(p, n, m, &s10),
        tail_conf(p, n, m, &s05),
    ]
}
...
pub fn run_stage3(g2: &[f64], y: &[f64], tau_list: &[f64], n_query: usize) -> Stage3Result {
    ...
    for &tau in tau_list.iter() {
        ...
        let cnt: Vec<f64> = (0..m)
            .map(|j| y.iter().filter(|&&v| v == classes[j]).count() as f64)
            .collect();
...
const FAMILIES_GRAPHS: [(&str, &[&str]); 8] = [
    ...
    ("ACTIVE", &["ACTIVE_EXACT"]),
    ("PASSIVE", &["PASSIVE_EXACT"]),
    ("PRICE", &["PRICE_T0", "PRICE_T2"]),
    ("SIDE", &["SIDE_EXACT"]),
    ("BOOK", &["BOOK_K10", "BOOK_K30"]),
    ("QUEUE", &["QUEUE_K10"]),
    ("ARRIVAL", &["ARRIVAL_K10"]),
];
...
fn gini(x: &[f64]) -> f64 {
    ...
    (2.0 * (1..=n).map(|i| i as f64 * s[i - 1]).sum::<f64>()) / (n as f64 * sum)
...
fn push(out: &mut Factors, name: &str, v: f64) {
...
pub fn target_dist(y: &[f64]) -> Factors {
    ...
    let z: Vec<f64> = if sdv > 1e-12 {
        y.iter().map(|v| (v - mu) / sdv).collect()
    } else {
        vec![0f64; n]
    };
    let skew = if sdv > 1e-12 {
        z.iter().map(|v| v.powi(3)).sum::<f64>() / n as f64
    } else {
        0.0
    };
    let kurt = if sdv > 1e-12 {
        z.iter().map(|v| v.powi(4)).sum::<f64>() / n as f64 - 3.0
    } else {
        0.0
    };
    ...
    let top_share = |frac: f64| {
        let k = ((frac * n as f64).ceil() as usize).max(1);
        sy[n - k..].iter().sum::<f64>() / (total.abs() + 1e-12)
    };
    let mut uniq = y.to_vec();
    uniq.sort_by(|a, b| a.partial_cmp(b).unwrap());
    uniq.dedup();
    let mut cnt: HashMap<i64, usize> = HashMap::new();
    for v in y.iter().map(|v| (v * 1e9) as i64) {
        *cnt.entry(v).or_default() += 1;
    }
    let shares: Vec<f64> = cnt.values().map(|&c| c as f64 / n as f64).collect();
    let mut f = vec![];
    let mut p = |n: &str, v: f64| push(&mut f, n, v);
    p("TARGET_MEAN", mu);
    p("TARGET_STD", sdv);
    p("TARGET_MEDIAN", med);
    p("TARGET_MAD", mad);
    ...
    p("TARGET_Q75", percentile(y, 75.0));
    p("TARGET_Q90", percentile(y, 90.0));
    p("TARGET_Q95", percentile(y, 95.0));
    p("TARGET_Q99", percentile(y, 99.0));
    p("TARGET_UNIQUE_COUNT", uniq.len() as f64);
    p("TARGET_UNIQUE_RATIO", uniq.len() as f64 / n as f64);
    p("TARGET_VALUE_HHI", shares.iter().map(|s| s * s).sum());
    p("TARGET_GINI", gini(y));
    p("TARGET_TOP1_SHARE", top_share(0.01));
    p("TARGET_TOP5_SHARE", top_share(0.05));
    p("TARGET_TOP10_SHARE", top_share(0.10));
    p(
        "TARGET_MAX_SHARE",
        y.iter().cloned().fold(f64::NEG_INFINITY, f64::max) / (total.abs() + 1e-12),
    );
    f
}

fn scale_features(level_ds: &[Vec<f64>]) -> (f64, f64) {
    if level_ds.is_empty() {
        return (0.0, 0.0);
    }
    let es: Vec<f64> = level_ds
        .iter()
        .map(|lv| lv.iter().map(|d| d * d).sum::<f64>())
        .collect();
...
pub fn s1_branches(s1: &Stage1Result) -> Factors {
    ...
    for b in &s1.branches {
        ...
        let vals = [
            ...
            net,
            sd(net, 1.0 - b.rho0),
            auc(&b.trajectory, b.rho0),
            sd(net, b.n_queries as f64),
            if b.n_internal > 0 {
                b.accepts.iter().filter(|&&x| x).count() as f64 / b.n_internal as f64
            } else {
                0.0
            },
            if b.node_ds.is_empty() {
                0.0
            } else {
                b.node_ds.iter().map(|d| d.abs()).sum::<f64>() / b.node_ds.len() as f64
            },
            if b.node_ds.is_empty() {
                0.0
            } else {
                b.node_ds.iter().map(|d| d * d).sum::<f64>() / b.node_ds.len() as f64
            },
            cent,
            sent,
            if b.parent_child_sign.is_empty() {
                0.0
            } else {
                b.parent_child_sign.iter().filter(|(a, c)| a == c).count() as f64
                    / b.parent_child_sign.len() as f64
            },
        ];
        for (suf, v) in S1_SUFFIX.iter().zip(vals.iter()) {
            push(&mut f, &format!("{}_{}", name, suf), *v);
        }
    }
    f
}
...
pub fn s1_summary(s1: &Stage1Result) -> Factors {
    ...
    p("S1_FINAL_CORR", s1.rho);
    p("S1_NET_GAIN", s1.rho - rho0);
    p("S1_NORM_GAIN", sd(s1.rho - rho0, 1.0 - rho0));
    ...
    p(
        "S1_BRANCH_GAIN_STD",
        (gains.iter().map(|g| (g - gm).powi(2)).sum::<f64>() / gains.len() as f64).sqrt(),
    );
    p(
        "S1_BRANCH_GAIN_MAX",
    ...
    p(
        "S1_BRANCH_GAIN_ENTROPY",
        if pstot > 1e-12 {
            entropy_norm(&pos.iter().map(|x| x / pstot).collect::<Vec<_>>(), 16)
        } else {
            0.0
        },
    );
    ...
    for fam in ["TIME", "ACTIVE", "PASSIVE", "PRICE"] {
        ...
        let g = fam_gains(fam);
        p(
            &format!("S1_{}_FAMILY_MEAN", fam),
            g.iter().sum::<f64>() / g.len() as f64,
        );
    }
    p("S1_ACTIVE_MINUS_TIME", fam_max("ACTIVE") - fam_max("TIME"));
    p(
        "S1_PASSIVE_MINUS_TIME",
        fam_max("PASSIVE") - fam_max("TIME"),
    );
    p("S1_PRICE_MINUS_TIME", fam_max("PRICE") - fam_max("TIME"));
    let coarse: f64 = s1
        .branches
        .iter()
        .filter(|b| b.depth == "N4" || b.depth == "N8")
        .map(|b| (b.rho_final - b.rho0).max(0.0))
        .sum();
    let fine: f64 = s1
        .branches
        .iter()
        .filter(|b| b.depth == "N32" || b.depth == "N16")
        .map(|b| (b.rho_final - b.rho0).max(0.0))
        .sum();
    let tot = coarse + fine + 1e-12;
    p("S1_COARSE_SHARE", coarse / tot);
    p("S1_FINE_SHARE", fine / tot);
    p("S1_COARSE_MINUS_FINE", (coarse - fine) / tot);
    for (i, beta) in [1, 2, 5, 10].iter().enumerate() {
    ...
    p(
        "S1_INIT_PATH_STD",
        (pr.iter().map(|x| (x - pmean).powi(2)).sum::<f64>() / pr.len() as f64).sqrt(),
    );
    p(
        "S1_INIT_PATH_RANGE",
        pr.iter().cloned().fold(f64::NEG_INFINITY, f64::max)
...
pub fn s2_all(s2: &Stage2Result, rho0: f64) -> Factors {
    ...
    for b in &s2.branches {
        ...
        let vals = [
            ...
            net,
            sd(net, 1.0 - rho1),
            auc(&b.trajectory, rho1),
            sd(net, b.n_queries as f64),
            if b.accepts.is_empty() {
                0.0
            } else {
                b.accepts.iter().filter(|&&x| x).count() as f64 / b.accepts.len() as f64
            },
            if b.node_ds.is_empty() {
                0.0
            } else {
        ...
        for (suf, v) in S2_SUFFIX.iter().zip(vals.iter()) {
            let nm = format!("{}_{}", name, suf);
            p(&nm, *v);
        }
    }
    let find = |g: &str, sc: &str| s2.branches.iter().find(|b| b.graph == g && b.scope == sc);
    for g in GRAPH_NAMES.iter() {
        let gains: Vec<f64> = SCOPES
            .iter()
            .map(|sc| find(g, sc).map(|b| b.rho_final - rho1).unwrap_or(0.0))
            .collect();
        p(
            &format!("S2_{}_GAIN_MAX_MODE", g),
            gains.iter().cloned().fold(f64::NEG_INFINITY, f64::max),
        );
        let gm = gains.iter().sum::<f64>() / 3.0;
        p(&format!("S2_{}_GAIN_MEAN_MODE", g), gm);
        p(
            &format!("S2_{}_GAIN_STD_MODE", g),
            (gains.iter().map(|x| (x - gm).powi(2)).sum::<f64>() / 3.0).sqrt(),
        );
        p(&format!("S2_{}_LOCAL_GAIN", g), gains[0]);
        p(&format!("S2_{}_GLOBAL_GAIN", g), gains[2]);
    ...
    for (fam, graphs) in FAMILIES_GRAPHS.iter() {
        ...
        for g in graphs.iter() {
            for sc in SCOPES.iter() {
                if let Some(b) = find(g, sc) {
                    gs.push(b.rho_final - rho1);
                }
            }
        }
        fam_gains.insert(fam, gs);
    }
    let all_pos: f64 = fam_gains
        .values()
        .flat_map(|gs| gs.iter().map(|g| g.max(0.0)))
        .sum::<f64>()
        + 1e-12;
    for fam in [
        "TIME", "ACTIVE", "PASSIVE", "PRICE", "SIDE", "BOOK", "QUEUE", "ARRIVAL",
    ] {
        let gs = &fam_gains[fam];
        p(
            &format!("S2_{}_FAMILY_GAIN_MAX", fam),
            gs.iter().cloned().fold(f64::NEG_INFINITY, f64::max),
        );
        p(
            &format!("S2_{}_FAMILY_GAIN_MEAN", fam),
            gs.iter().sum::<f64>() / gs.len() as f64,
        );
        p(
            &format!("S2_{}_FAMILY_SHARE", fam),
            gs.iter().map(|g| g.max(0.0)).sum::<f64>() / all_pos,
        );
        let gm = gs.iter().sum::<f64>() / gs.len() as f64;
        p(
            &format!("S2_{}_FAMILY_PARAM_STD", fam),
            if gs.len() > 1 {
                (gs.iter().map(|g| (g - gm).powi(2)).sum::<f64>() / gs.len() as f64).sqrt()
            } else {
                0.0
            },
        );
    ...
    let fmax2 = |fam: &str| -> f64 {
        ...
        s2.branches
            .iter()
            .filter(|b| graphs.contains(&b.graph.as_str()))
            .map(|b| b.rho_final - rho1)
            .fold(f64::NEG_INFINITY, f64::max)
    };
    for (a, b) in [
        ("ACTIVE", "TIME"),
    ...
    p(
        "S2_ACTIVE_TO_TIME_RATIO",
        sd(fmax2("ACTIVE"), fmax2("TIME").abs()),
    );
    p(
        "S2_PASSIVE_TO_TIME_RATIO",
        sd(fmax2("PASSIVE"), fmax2("TIME").abs()),
    );
    p(
        "S2_BOOK_TO_PRICE_RATIO",
        sd(fmax2("BOOK"), fmax2("PRICE").abs()),
    );
    p(
        "S2_QUEUE_TO_BOOK_RATIO",
        sd(fmax2("QUEUE"), fmax2("BOOK").abs()),
    );
    p("S2_FINAL_CORR", s2.rho);
    p("S2_INCREMENTAL_GAIN", s2.rho - rho1);
    p("S2_TOTAL_GAIN", s2.rho - rho0);
    ...
    p(
        "S2_GRAPH_GAIN_STD",
        (gg.iter().map(|g| (g - ggm).powi(2)).sum::<f64>() / gg.len() as f64).sqrt(),
    );
    p(
        "S2_GRAPH_GAIN_MAX",
        gg.iter().cloned().fold(f64::NEG_INFINITY, f64::max),
    );
    p(
        "S2_GRAPH_POS_RATIO",
        gg.iter().filter(|&&g| g > 0.0).count() as f64 / gg.len() as f64,
    );
    let shares: Vec<f64> = [
        "TIME", "ACTIVE", "PASSIVE", "PRICE", "SIDE", "BOOK", "QUEUE", "ARRIVAL",
    ]
    .iter()
    .map(|fam| fmax2(fam).max(0.0))
    .collect();
    ...
    let mut so = (0..8).collect::<Vec<usize>>();
    so.sort_by(|&a, &b| shares[b].partial_cmp(&shares[a]).unwrap());
    p("S2_WINNER_MARGIN", (shares[so[0]] - shares[so[1]]) / stot);
    p(
        "S2_TOP2_FAMILY_SHARE",
        (shares[so[0]] + shares[so[1]]) / stot,
    );
    let lp: f64 = s2
        .branches
        .iter()
        .filter(|b| b.scope == "LOCAL32")
        .map(|b| (b.rho_final - rho1).max(0.0))
        .sum();
    let mp: f64 = s2
        .branches
        .iter()
        .filter(|b| b.scope == "MESO8_D2")
        .map(|b| (b.rho_final - rho1).max(0.0))
        .sum();
    let gp: f64 = s2
        .branches
        .iter()
        .filter(|b| b.scope == "GLOBAL_D4")
        .map(|b| (b.rho_final - rho1).max(0.0))
        .sum();
    let stt = lp + mp + gp + 1e-12;
    p("S2_LOCAL_SHARE", lp / stt);
    p("S2_MESO_SHARE", mp / stt);
    p("S2_GLOBAL_SHARE", gp / stt);
...
pub fn s3_contrib_cross(
    s1: &Stage1Result,
    s2: &Stage2Result,
    s3: Option<&Stage3Result>,
) -> Factors {
    let mut f = vec![];
    let mut p = |n: &str, v: f64| push(&mut f, n, v);
    if let Some(s3) = s3 {
        for t in s3.taus.iter() {
            ...
            for &ck in &[0usize, 8, 16, 32, 64] {
                if let Some(stats) = t.checkpoints.get(&ck) {
                    for (stat, idx) in S3_STATS.iter() {
                        p(&format!("S3_{}_K{}_{}", tn, ck, stat), stats[*idx]);
                    }
                }
            }
            ...
            p(
                &format!("S3_{}_EARLY_INFO_SHARE8", tn),
                sd(h0 - h8, h0 - h64),
            );
            p(
                &format!("S3_{}_EARLY_INFO_SHARE16", tn),
                sd(h0 - h16, h0 - h64),
            );
            let ig = &t.info_gains;
            p(
                &format!("S3_{}_QUERY_INFO_GAIN_MAX", tn),
                ig.iter().cloned().fold(f64::NEG_INFINITY, f64::max),
            );
            let igm = ig.iter().sum::<f64>() / ig.len() as f64;
            p(&format!("S3_{}_QUERY_INFO_GAIN_MEAN", tn), igm);
            p(
                &format!("S3_{}_QUERY_INFO_GAIN_STD", tn),
                (ig.iter().map(|x| (x - igm).powi(2)).sum::<f64>() / ig.len() as f64).sqrt(),
            );
            p(&format!("S3_{}_QUERY_INFO_GAIN_GINI", tn), gini(ig));
            p(
                &format!("S3_{}_PREDICTED_VARIANCE_MEAN", tn),
                t.pred_vars.iter().sum::<f64>() / t.pred_vars.len() as f64,
            );
            p(&format!("S3_{}_FINAL_POSTERIOR_CORR", tn), t.rho3);
            p(&format!("S3_{}_FINAL_GAIN_VS_STAGE2", tn), t.rho3 - t.rho2);
            for (tp_i, tp) in ["DISAGREEMENT", "STRUCTURE", "ENTROPY", "RANDOM"]
                .iter()
                .enumerate()
            {
                let cnt = t
                    .query_types
                    .iter()
                    .filter(|&&q| q as usize == tp_i)
                    .count();
                p(
                    &format!("S3_{}_{}_SHARE", tn, tp),
                    cnt as f64 / t.query_types.len() as f64,
                );
                let g: Vec<f64> = t
                    .query_types
                    .iter()
                    .enumerate()
                    .filter(|(_, &q)| q as usize == tp_i)
                    .map(|(i, _)| ig[i])
                    .collect();
                p(
                    &format!("S3_{}_{}_GAIN_MEAN", tn, tp),
                    if g.is_empty() {
                        0.0
                    } else {
                        g.iter().sum::<f64>() / g.len() as f64
    ...
    p("X_S1_SHARE_12", sd(g1, g1 + g2));
    p("X_S2_SHARE_12", sd(g2, g1 + g2));
    ...
    let s1_coarse = coarse / ctot;
    let s1_fine = fine / ctot;
    let rho1 = s2.rho1;
    ...
    p(
        "X_ORDER_IDENTITY_DOMINANCE",
        (fam_gmax("ACTIVE") + fam_gmax("PASSIVE")) / 2.0 - fam_gmax("TIME"),
    );
    p(
        "X_BOOK_QUEUE_DOMINANCE",
        (fam_gmax("BOOK") + fam_gmax("QUEUE")) / 2.0 - fam_gmax("PRICE"),
    );
    if let Some(s3) = s3 {
        for t in s3.taus.iter() {
            let tn = format!("TAU{:03}", (t.tau * 100.0) as i64);
            ...
            p(
                &format!("X_ACTIVE_GAIN_X_TAIL10_ENTROPY_{}", tn),
                fam_gmax("ACTIVE") * ckl[8],
            );
            p(
                &format!("X_BOOK_GAIN_X_TAIL10_CONF_{}", tn),
                fam_gmax("BOOK") * ckl[11],
            );
            p(
                &format!("X_GRAPH_ENTROPY_X_TOTAL_INFO_{}", tn),
                s2_me * total_info,
            );
            p(
                &format!("X_SCALE_ENTROPY_X_FINAL_ENTROPY_{}", tn),
                s1_bge * ckl[0],
            );
            p(
                &format!("X_HIDDEN_LARGE_ORDER_{}", tn),
                active_minus_time * ckl[9],
            );
            p(
                &format!("X_VISIBLE_LARGE_LIQUIDITY_{}", tn),
                book_minus_time * ckl[12],
            );
        }
    }
    f
