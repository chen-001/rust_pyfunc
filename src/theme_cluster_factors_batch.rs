...
fn kmeans_flat(
    ...
    kmeans_flat_with_init(data, n, d, k, n_init, max_iter, None)
}

/// KMeans聚类 (支持warm-start初始中心)
fn kmeans_flat_with_init(
    data: &[f64],
    n: usize,
    d: usize,
    k: usize,
    ...
    // 如果有前一天的中心，额外多跑一轮warm-start
    let total_inits = if prev_centers.is_some() {
        n_init + 1
    } else {
        n_init
    };

    for init in 0..total_inits {
        let mut centers = if init == 0 && prev_centers.is_some() {
            // 第一轮用前一天的中心做warm-start
            let pc = prev_centers.unwrap();
            if pc.len() == k * d {
                pc.to_vec()
            } else {
                // 维度不匹配时退回kmeans++
                let seed = 42 + init as u64 * 1000;
                kmeans_pp_init_flat(data, n, d, k, seed)
            }
        } else {
            let actual_init = if prev_centers.is_some() {
                init - 1
            } else {
                init
            };
            let seed = 42 + actual_init as u64 * 1000;
            kmeans_pp_init_flat(data, n, d, k, seed)
        };
...
/// 单日聚类 (返回缓存结构，支持warm-start)
fn cluster_single_day_cached(features: &[f64], n: usize, d: usize, k: usize) -> CachedDayResult {
    cluster_single_day_cached_warmstart(features, n, d, k, None)
}

/// 单日聚类 (warm-start版本，用前一天的标准化后中心做初始化)
fn cluster_single_day_cached_warmstart(
    features: &[f64],
    n: usize,
    d: usize,
    k: usize,
    ...
    // KMeans聚类 (warm-start: 用前一天的中心做初始化)
    let (labels_clean, centers) =
        kmeans_flat_with_init(&scaled, n_valid, d, actual_k, 10, 300, prev_centers);
    ...
    CachedDayResult {
        labels: full_labels,
        centers: Array2::from_shape_vec((actual_k, d), centers)
            .unwrap_or_else(|_| Array2::zeros((actual_k, d))),
        features_clean: Array2::from_shape_vec((n_valid, d), clean)
            .unwrap_or_else(|_| Array2::zeros((n_valid, d))),
        valid_indices,
        stats,
        k: actual_k,
    }
...
fn align_two_days_opt(
    ...
    let mapping_pairs: Vec<(usize, usize)> = match method {
        ...
        _ => {
            ...
            let cost = compute_distance_matrix_flat(
                curr_centers,
                prev_centers,
                k1,
                k2,
            ...
            let cost_arr =
                Array2::from_shape_vec((k1, k2), cost).unwrap_or_else(|_| Array2::zeros((k1, k2)));
    ...
    if distance_threshold > 0.0 && method != "overlap" {
        ...
        let cost =
            compute_distance_matrix_flat(curr_centers, prev_centers, k1, k2, d, distance_metric);
        ...
        for c in 0..k {
            if !prev_theme_stocks[c].is_empty() {
                let sum: f64 = prev_theme_stocks[c]
                    .iter()
                    .filter_map(|&s| {
                        if s < prev_micro.len() && prev_micro[s].is_finite() {
                            Some(prev_micro[s])
                        } else {
                            None
                        }
...
fn compute_single_factor_set(
    ...
    let stats_vec: Vec<ClusterStats> = (0..n_days)
        .map(|d| {
            ...
            let labels_for_clean: Vec<usize> =
                valid.iter().map(|&vi| aligned_labels[d][vi]).collect();
    ...
    // 计算各因子
    let f_momentum =
        factor_theme_momentum_opt(&theme_history, n_stocks, n_days, &stats_vec, lookback);
    ...
    let f_rank = factor_theme_rank_opt(
        target_features,
        target_labels,
        target_centers,
        n_stocks,
    ...
    let f_evolution = factor_theme_evolution_opt(
        &theme_history,
        n_stocks,
        n_days,
        &stats_vec,
    ...
    if n_micro > 0 && n_days >= 1 {
        ...
        for m_idx in 0..n_micro {
            let curr_metric: Vec<f64> = micro_arrays[target_day]
                .column(m_idx)
                .iter()
                .cloned()
                .collect();
...
pub fn theme_cluster_factors_batch(
    ...
    for &k in &k_values_unique {
        ...
        for day_idx in 0..n_days {
            ...
            let prev_centers_flat: Option<Vec<f64>> = if day_idx > 0 {
                let prev = &day_results[day_idx - 1];
                if prev.k == k {
                    Some(prev.centers.as_slice().unwrap_or(&[]).to_vec())
                } else {
                    None
                }
            } else {
            ...
            let result = cluster_single_day_cached_warmstart(
                features,
                n_stocks,
                d,
                k,
                prev_centers_flat.as_deref(),
            );
...
pub fn theme_cluster_factors_batch_multi_segments(
    ...
    for (seg_name, feat_list, micro_list) in segments_data {
        ...
        let feat_arrays: Vec<Array2<f64>> =
            feat_list.iter().map(|f| f.as_array().to_owned()).collect();
        let micro_arrays: Vec<Array2<f64>> =
            micro_list.iter().map(|f| f.as_array().to_owned()).collect();
    ...
    let base_factor_names = vec![
        "theme_return_mean",
        "yesterday_theme_return",
        "theme_return_trend",
        "theme_size",
        "theme_amount",
        "theme_return",
        "theme_act_buy_ratio",
        "return_rank_in_theme",
        "distance_to_center",
        "switch_direction",
        "switch_strength",
        "theme_entropy",
        "theme_heat",
        "theme_momentum_persistence",
        "theme_diffusion",
        "leader_stability",
    ];
    let micro_metric_names = vec![
    ...
    // 预计算各时段各k值的聚类结果
    let mut cluster_cache_by_segment: HashMap<String, HashMap<usize, Vec<CachedDayResult>>> =
        HashMap::new();
    ...
    let results: Vec<(usize, usize, Vec<f64>)> = pool.install(|| {
        tasks
            .into_par_iter()
            .map(
                |(seg_idx, combo_idx, seg_name, k, align, dist, thresh, lb)| {
    ...
    for (seg_idx, (seg_name, _, _, _)) in segment_info.iter().enumerate() {
        for (combo_idx, (k, align, dist, thresh, lb)) in param_combos.iter().enumerate() {
            ...
            // 找到对应结果并填充
            if let Some((_, _, result)) = results
                .iter()
                .find(|(si, ci, _)| *si == seg_idx && *ci == combo_idx)
            {
                for s in 0..n_stocks {
                    for f in 0..n_factors {
...
pub fn theme_cluster_factors_from_minute(
        ...
        for s in 0..n_stocks {
            ...
            if len > 1 {
                ...
                if log_rets.len() > 1 {
                    ...
                    let var: f64 = log_rets.iter().map(|x| (x - mean).powi(2)).sum::<f64>()
                        / log_rets.len() as f64;
    ...
    let extract_micro = |day_data: &[f64], start: usize, end: usize| -> Vec<f64> {
        ...
        for s in 0..n_stocks {
            ...
            let sum_field = |field: usize| -> f64 { (start..end).map(|m| get_val(field, m)).sum() };
            let mean_field = |field: usize| -> f64 {
    ...
    // 预计算各时段各k值的聚类结果 (warm-start: 前一天的中心传给下一天)
    let mut cluster_cache: Vec<HashMap<usize, Vec<CachedDayResult>>> =
    ...
    let results: Vec<(usize, usize, Vec<f64>)> = pool.install(|| {
        tasks
            ...
            .map(|(seg_idx, combo_idx, k, align, dist, thresh, lb)| {
                ...
                let features_arrays: Vec<Array2<f64>> = seg_features[seg_idx]
                    .iter()
                    .map(|f| Array2::from_shape_vec((n_stocks, N_FEATURES), f.clone()).unwrap())
                    .collect();
                let micro_arrays: Vec<Array2<f64>> = seg_micro[seg_idx]
                    .iter()
    ...
    for seg_idx in 0..n_segments {
        ...
        for (combo_idx, (k, align, dist, thresh, lb)) in param_combos.iter().enumerate() {
            ...
            let param_suffix = format!(
                "__k{}_{}_{}_{}_lb{}__{}",
                k, align, dist, thresh_str, lb, seg_name
            );
