...
fn calculate_price_direction_seq(prices: &[f64], window_size: usize) -> Vec<Vec<i32>> {
    ...
    for i in 0..price_changes.len() {
        ...
        let window: Vec<i32> = price_changes[start..=i]
            .iter()
            .map(|&v| {
                if v > 1e-10 {
                    1
...
pub fn calculate_microstructure_pattern_features(
    ...
    let direction_patterns: Vec<(&str, Vec<Vec<i32>>)> =
        vec![("price_direction", price_direction_seq.clone())];

    for (pattern_name, pattern_values) in &direction_patterns {
        for (dist_name, dist_type) in &rank_distances {
            for (cons_name, cons_type) in &constructions {
                let sequence = match cons_type {
                    ConstructionType::MovingSplitPoint => construct_moving_split_point_direction(
                        pattern_values,
                        window_size,
                        *dist_type,
                    ),
                    ConstructionType::AdjacentRollingWindows => {
                        construct_adjacent_rolling_windows_direction(
                            pattern_values,
                            window_size,
                            *dist_type,
                        )
                    ...
                    ConstructionType::CumulativeVsMarginal => {
                        construct_cumulative_vs_marginal_direction(
                            pattern_values,
                            marginal_window,
                            *dist_type,
                        )
