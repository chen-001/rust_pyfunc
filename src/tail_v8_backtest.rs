...
use crate::tail_v5_pipeline::{
    ...
    nanmean_f64, nanstd_population, rank_both_radix, BtPrecomputed, LegacyBacktestResult, EPS,
};
    ...
    pub fn push_block(&mut self, block: &ArrayView2<f32>, t0: usize) {
        ...
        for r in 0..rows {
            ...
            for s in 0..n {
                ...
                let rv = self.ctx.restrict[[self.held_restrict_row, s]];
                ...
                if signal_value.is_finite() && ret_value.is_finite() && is_open {
                    ...
                    self.filtered_stock_idx.push(s as u32);
                }
            }
            ...
            if (local_t + 1) % gap == 0 {
                ...
                let (xx, avg_now) = rank_both_radix(&self.filtered_signal);
                ...
                for idx in 0..self.filtered_signal.len() {
                    let diff = self.walk_buf[idx] - xx[idx];
            ...
            let ranks = avg_signal.unwrap_or_else(|| rank_both_radix(&self.filtered_signal).1);
