...
pub fn compute_pair_interaction_full(date: i64) -> std::io::Result<(Vec<String>, Vec<f32>)> {
    ...
    let preps: Vec<Option<StockData>> = codes
        .par_iter()
        .map(|c| read_trade_fast_inner(c, date, false, true, usize::MAX).ok().and_then(|recs| per_stock_prep(c, &recs, day_start_us)))
        .collect();
    let stocks: Vec<StockData> = preps.into_iter().flatten().collect();
    let n = stocks.len();
    let total_trades: usize = stocks.iter().map(|s| s.n).sum();
    eprintln!("[pair_interaction] valid = {n}, trades = {total_trades}, read+prep {:.1}s", t0.elapsed().as_secs_f32());
    ...
    eprintln!("[pair_interaction] merge {:.1}s", t0.elapsed().as_secs_f32());
    ...
    eprintln!("[pair_interaction] X4/X5/X6 {:.1}s", t0.elapsed().as_secs_f32());
    ...
    eprintln!("[pair_interaction] derived+combos {:.1}s", t0.elapsed().as_secs_f32());
    ...
    let spectral_mats: Vec<(String, Vec<f32>)> = spec_names
        ...
        .collect();
    eprintln!("[pair_interaction] spectral {:.1}s", t_spec.elapsed().as_secs_f32());

    // ---- ⑦ 因子提取 ----
    let t0 = std::time::Instant::now();
    let (names, vals) = extract_factors(n, &reg, &combos, &spectral_mats, &stocks);
    eprintln!("[pair_interaction] factors {:.1}s, n_factors = {}", t0.elapsed().as_secs_f32(), names.len());
    assert_eq!(names.len(), factor_names().len(), "因子名目录与计算不一致");

    let codes_out: Vec<String> = stocks.iter().map(|s| s.code.clone()).collect();
    eprintln!("[pair_interaction] total {:.1}s", t_all.elapsed().as_secs_f32());
    Ok((codes_out, vals))
}
...
fn matrix_order() -> Vec<(String, bool)> {
    ...
    for sname in ["30s", "60s", "300s"] { order.push((format!("x4_{sname}"), false)); }
    for sname in ["30s", "60s", "300s"] {
        order.push((format!("x5lag_{sname}"), true));
        order.push((format!("x5str_{sname}"), true));
    }
    for sname in ["30s", "60s", "300s"] { order.push((format!("x6_{sname}"), true)); }
    for sname in ["10s", "30s"] { order.push((format!("x7_{sname}"), false)); }
    for (nm, _) in COMBO_DEFS.iter() { order.push((nm.to_string(), false)); }
    order
}

fn spectral_names() -> Vec<String> {
    let sym: [&str; 9] = ["x1", "x3", "x8", "x9", "x4_30s", "x4_60s", "x4_300s", "x7_10s", "x7_30s"];
...
const SELF_NAMES: [&str; 21] = [
    "cnt1_ac60", "cnt1_ac300", "cnt1_ac900",
    "s60_ac1", "s60_ac5", "s60_ac30", "s30_ac1", "s300_ac1",
    "ret60_std", "ret60_skew", "ret60_kurt", "active_min_ratio", "open_close_ratio",
    "log_n", "log_turnover", "log_median_vol", "log_th99", "big_fill_ratio",
    "avg_trade_size", "nbig", "s60_pos_min_ratio",
];
