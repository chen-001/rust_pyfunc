//! 跨股票互动因子流水线（cross-section-pipeline 规范）
//!
//! 读一天全市场 Level2 逐笔 → per-stock 预处理（重采样到统一时间网格，B 类因子规范）
//! → 两两互动矩阵（X1-X9，21+6 个 N×N 矩阵）→ 五层因子提取（行画像/谱嵌入/图结构/
//! 矩阵交互/邻域跨矩阵）→ 每股 6694 个因子。
//!
//! 性能（20241231, 50 核, 实测 ~54s, 含读取）：
//!   - 逐笔时间毫秒打包 u32（24bit），合并流带宽最小化
//!   - X1/X3/X8/X9 只算上三角；X2/X5 反向 = 转置（一次 gemm）
//!   - 手写分块 f32 gemm（Bᵀ 转置 + i-block 并行）
//!   - 谱分解用确定性幂迭代（固定种子 + GS 正交化）
//!   - 二跳特征 O(N²)（diag=行平方和, row_mean=矩阵×列和），不做 N³ 矩阵乘
//!   - universe = 按文件大小（成交活跃度代理）取 top-2500（"几千×几千"矩阵）
//!
//! 注册：lib.rs (mod + py_pair_interaction + py_pair_interaction_names)、
//!       factor_pipeline.rs (pipeline_pair_interaction + known)、
//!       worker_pipeline.rs (分发)、__init__.pyi
...
mod gemm {
    //! 手写分块 f32 GEMM（C[m×n] = A[m×k]·Bᵀ[n×k]），B 预转置，i-block 并行
    use rayon::prelude::*;
...
fn per_stock_prep(code: &str, recs: &[TradeRecord], day_start_us: i64) -> Option<StockData> {
    ...
    let recs: Vec<&TradeRecord> = recs.iter().filter(|r| r.time_us >= day_start_us).collect();
    let n = recs.len();
    if n < MIN_TRADES {
        return None;
    }
    ...
    let mut s30 = vec![0f32; SCALE_30];
    let mut p30 = vec![0f32; SCALE_30];
    let mut s60 = vec![0f32; SCALE_60];
    let mut p60 = vec![0f32; SCALE_60];
    let mut c60 = vec![0f32; SCALE_60];
    let mut s300 = vec![0f32; SCALE_300];
    let mut p300 = vec![0f32; SCALE_300];
    ...
    fills.sort_by_key(|f| f.0);
    {
        ...
        for &(oid, t, vol, sign) in &fills {
            while ti + 1 < totals.len() && totals[ti].0 < oid {
                ti += 1;
            }
...
fn merge_x139_multi(a: &[u32], bs: &[&[u32]], out: &mut [(f32, f32, f32)]) {
    ...
    for (k, o) in out.iter_mut().enumerate() {
        ...
        let x9 = if s.gcnt > 1 && s.gsum > 0 {
            ...
            let var = (s.gsq as f64 - s.gsum as f64 * s.gsum as f64 / s.gcnt as f64)
                / (s.gcnt - 1) as f64;
            if var > 0.0 {
                (var.sqrt() / mean) as f32
            } else {
...
fn merge_x8(a: &[u32], b: &[u32]) -> f32 {
    ...
    while ia < a.len() || ib < b.len() {
        ...
        if last != u32::MAX {
            ...
            let bin = if gap_ms >= 320_000 {
                32usize
            } else {
                (gap_ms / 10_000) as usize
            };
            h[bin] += 1;
            ngaps += 1;
        }
    ...
    if ngaps > 0 {
        (h[0] + h[1] + h[2]) as f32 / ngaps as f32
    } else {
        0.0
    }
}
...
// 主流程
// ============================================================
/// 核心唯一真相源：读全市场 → per-stock → 两两互动矩阵 → 因子提取 → (codes, vals)
pub fn compute_pair_interaction_full(date: i64) -> std::io::Result<(Vec<String>, Vec<f32>)> {
    let t_all = std::time::Instant::now();
    let codes = list_codes(date);
    eprintln!("[pair_interaction] codes = {}", codes.len());
    ...
    let preps: Vec<Option<StockData>> = codes
        .par_iter()
        .map(|c| {
            read_trade_fast_inner(c, date, false, true, usize::MAX)
                .ok()
                .and_then(|recs| per_stock_prep(c, &recs, day_start_us))
        })
        .collect();
    ...
    eprintln!(
        "[pair_interaction] valid = {n}, trades = {total_trades}, read+prep {:.1}s",
    ...
    drop(c_dense);
    eprintln!("[pair_interaction] X2 {:.1}s", t0.elapsed().as_secs_f32());
    ...
    for (si, (sname, t, kk)) in [
        ("30", SCALE_30, 10usize),
        ("60", SCALE_60, 5usize),
        ("300", SCALE_300, 1usize),
    ]
    .iter()
    .enumerate()
    {
        ...
        for i in 0..n {
            zr[i * (t_trim - 1)..(i + 1) * (t_trim - 1)]
                .copy_from_slice(&r[i * t_trim + 1..i * t_trim + t_trim]);
        }
        drop(r);
        zscore_rows(&mut zr, n, t_trim - 1);
        let mut zs1 = vec![0f32; n * (t_trim - 1)];
        for i in 0..n {
            zs1[i * (t_trim - 1)..(i + 1) * (t_trim - 1)]
                .copy_from_slice(&s2[i * t_trim..i * t_trim + t_trim - 1]);
        }
    ...
    for (si, t) in [BIG10_T, BIG30_T].iter().enumerate() {
        ...
        for i in 0..n {
            for j in 0..n {
                ...
                x7[i * n + j] = if denom > 0.0 {
                    (x7[i * n + j] + tmp[i * n + j]) / denom
                } else {
                    0.0
                };
            }
    ...
    eprintln!("[pair_interaction] X7 {:.1}s", t0.elapsed().as_secs_f32());
    ...
    eprintln!(
        "[pair_interaction] derived+combos {:.1}s",
        t0.elapsed().as_secs_f32()
    );

    let sym_names: [&str; 9] = [
        "x1", "x3", "x8", "x9", "x4_30s", "x4_60s", "x4_300s", "x7_10s", "x7_30s",
    ];
    let spec_names: Vec<&str> = sym_names
        .iter()
        .copied()
        .chain(combo_defs.iter().map(|(nm, _)| *nm))
        .collect();
    let t_spec = std::time::Instant::now();
    let spectral_mats: Vec<(String, Vec<f32>)> = spec_names
        .par_iter()
        .map(|nm| {
            let m = reg.get(*nm).unwrap();
        ...
        .collect();
    eprintln!(
        "[pair_interaction] spectral {:.1}s",
        t_spec.elapsed().as_secs_f32()
    );
    ...
    eprintln!(
        "[pair_interaction] factors {:.1}s, n_factors = {}",
        t0.elapsed().as_secs_f32(),
        names.len()
    );
    assert_eq!(names.len(), factor_names().len(), "因子名目录与计算不一致");

    let codes_out: Vec<String> = stocks.iter().map(|s| s.code.clone()).collect();
    eprintln!(
        "[pair_interaction] total {:.1}s",
        t_all.elapsed().as_secs_f32()
    );
    Ok((codes_out, vals))
}

/// 因子名目录（不计算数值；与 extract_factors 的顺序严格一致）
pub fn factor_names() -> Vec<String> {
    let mut names: Vec<String> = Vec::new();
    let order = matrix_order();
    for (nm, asym) in order.iter() {
        for s in row_feature_names(*asym) {
            names.push(format!("{nm}__{s}"));
        }
    }
    let spec_names = spectral_names();
    for nm in spec_names.iter() {
        for j in 0..SPECTRAL_K {
            names.push(format!("spec_{nm}__ev{}", j + 1));
        }
    }
    for nm in GRAPH_NAMES.iter() {
        names.push(format!("graph_{nm}__wdeg"));
        names.push(format!("graph_{nm}__deg"));
        names.push(format!("graph_{nm}__pagerank"));
        names.push(format!("graph_{nm}__clustering"));
        for j in 0..15 {
            names.push(format!("graph_{nm}__ev{}", j + 1));
        }
    }
    let derived_names = derived_matrix_names();
    for nm in derived_names.iter() {
        for s in ["mean", "std", "topk5_mean", "topk50_mean", "max_abs", "net"] {
            names.push(format!("der_{nm}__{s}"));
        }
    }
    for (a, b) in PROD_PAIRS.iter() {
        for s in ["mean", "std", "topk5_mean", "topk50_mean", "max"] {
            names.push(format!("prod_{a}_{b}__{s}"));
        }
    }
    for (a, b) in HOP_PAIRS.iter() {
        names.push(format!("hop2_{a}_{b}__diag"));
        names.push(format!("hop2_{a}_{b}__row_mean"));
    }
    for (a, b) in COS_PAIRS.iter() {
        names.push(format!("cos_{a}_{b}"));
    }
    let o = matrix_order();
    for mi in 0..o.len() {
        for ni in 0..o.len() {
            ...
            let nm_m = &o[mi].0;
            let nm_n = &o[ni].0;
            for &k in [5usize, 20usize].iter() {
                names.push(format!("nb_{nm_m}_{nm_n}_top{k}"));
                names.push(format!("nb_{nm_m}_{nm_n}_bot{k}"));
            }
            names.push(format!("nb_{nm_m}_{nm_n}_ratio20"));
        }
    }
    for s in SELF_NAMES.iter() {
        names.push(format!("self_{s}"));
    }
    names
}

/// 矩阵顺序（与 extract_factors 的 order 构建一致）
fn matrix_order() -> Vec<(String, bool)> {
    let mut order: Vec<(String, bool)> = Vec::new();
    for nm in ["x1", "x3", "x8", "x9"] {
        order.push((nm.to_string(), false));
    }
    for dt in X2_DT.iter() {
        order.push((format!("x2_{dt}s"), true));
    }
    ...
    for sname in ["10s", "30s"] {
        order.push((format!("x7_{sname}"), false));
    }
    for (nm, _) in COMBO_DEFS.iter() {
        order.push((nm.to_string(), false));
    }
    order
}

fn spectral_names() -> Vec<String> {
    let sym: [&str; 9] = [
        "x1", "x3", "x8", "x9", "x4_30s", "x4_60s", "x4_300s", "x7_10s", "x7_30s",
    ];
    let mut v: Vec<String> = sym.iter().map(|s| s.to_string()).collect();
    for (nm, _) in COMBO_DEFS.iter() {
        v.push(nm.to_string());
    }
    v
...
fn derived_matrix_names() -> Vec<String> {
    let mut v: Vec<String> = Vec::new();
    for dt in X2_DT.iter() {
        v.push(format!("x2_{dt}s_sum"));
        v.push(format!("x2_{dt}s_asy"));
    }
    for pair in [(10usize, 30usize), (10, 60), (30, 60)] {
        let (a, b) = pair;
        v.push(format!("x2_{a}s_sum_m_{b}s_sum"));
        v.push(format!("x2_{a}s_asy_m_{b}s_asy"));
    }
    for sname in ["30s", "60s", "300s"] {
        v.push(format!("x5lagxstr_{sname}"));
    }
    v.push("x6_60s_asy".into());
    v.push("x4_30s_m_300s".into());
    v.push("x7_10s_m_30s".into());
    v.push("x5str_30s_m_300s".into());
    v
}

const COMBO_DEFS: [(&str, [&str; 3]); 6] = [
    ("combo1", ["x4_60s", "x7_30s", "x1"]),
    ("combo2", ["x2_10s_sum", "x5str_60s", "x6_60s"]),
    ("combo3", ["x3", "x8", "x9"]),
    ("combo4", ["x4_300s", "x5str_300s", "x2_60s_sum"]),
    ("combo5", ["x4_30s", "x7_10s", "x2_30s_sum"]),
    ("combo6", ["x5str_30s", "x5str_300s", "x4_60s"]),
];

const PROD_PAIRS: [(&str, &str); 70] = [
    ("x4_60s", "x7_30s"),
    ("x4_30s", "x7_30s"),
    ("x4_300s", "x7_30s"),
    ("x4_60s", "x7_10s"),
    ...
    ("combo2", "combo4"),
    ("combo5", "combo6"),
    ("x2_30s_sum", "x5str_60s"),
    ("x2_10s_asy", "x2_10s_asy"),
    ("x2_10s_sum", "x5str_60s"),
    ("x4_30s", "x5lag_30s"),
    ("x8", "x9"),
    ("x1", "x2_10s_sum"),
    ("x9", "x7_30s"),
    ("x2_10s", "x4_30s"),
    ("x2_60s", "x4_300s"),
    ("x5str_30s", "x5str_300s"),
];

const HOP_PAIRS: [(&str, &str); 40] = [
    ("x4_60s", "x4_60s"),
    ("x7_30s", "x7_30s"),
    ("x1", "x1"),
    ("x3", "x3"),
    ...
    ("x4_300s", "x4_300s"),
    ("x7_10s", "x7_10s"),
    ("x2_10s_sum", "x2_10s_sum"),
    ("x2_60s_sum", "x2_60s_sum"),
    ("x5str_60s", "x5str_60s"),
    ("combo1", "combo1"),
    ("combo2", "combo2"),
    ("combo3", "combo3"),
...
// ============================================================
#[pyfunction]
pub fn py_pair_interaction(py: Python<'_>, date: i64) -> PyResult<(Vec<String>, Vec<f32>)> {
    py.allow_threads(|| {
        compute_pair_interaction_full(date)
            .map_err(|e| pyo3::exceptions::PyIOError::new_err(format!("{e:?}")))
    })
}

#[pyfunction]
pub fn py_pair_interaction_names() -> Vec<String> {
    factor_names()
}
...
// ============================================================
const TOPK: [usize; 19] = [
    1, 2, 3, 5, 8, 10, 15, 20, 30, 50, 80, 100, 150, 200, 300, 500, 800, 1200, 2000,
];
...
fn row_feature_names(asym: bool) -> Vec<String> {
    let mut names: Vec<String> = Vec::new();
    for s in [
        "mean", "std", "skew", "kurt", "min", "max", "median", "iqr", "q01", "q02", "q05", "q10",
        "q25", "q75", "q90", "q95", "q98", "q99",
    ] {
...
fn row_features(m: &[f32], n: usize, asym: bool, prefix: &str) -> RowFeats {
    ...
    for i in 0..n {
        ...
        let skew = if std > 1e-12 {
            s3 / nn as f64 / std.powi(3)
        } else {
            0.0
        };
        let kurt = if std > 1e-12 {
            s4 / nn as f64 / std.powi(4) - 3.0
        } else {
            0.0
        };
        ...
        let mut posv: Vec<f32> = Vec::new();
        let mut negv: Vec<f32> = Vec::new();
        for x in r.iter() {
            if x.0 > 0.0 {
                cnt_pos += 1;
                sum_pos += x.0 as f64;
        ...
        let top3p = if posv.len() >= 3 {
            ...
            posv.iter().map(|&x| x as f64).sum::<f64>() / posv.len().max(1) as f64
        };
        let top3n = if negv.len() >= 3 {
            negv[negv.len() - 3..]
                .iter()
                .map(|&x| x as f64)
                .sum::<f64>()
            ...
            negv.iter().map(|&x| x as f64).sum::<f64>() / negv.len().max(1) as f64
        };
        feats.extend([
            mean_abs,
            max_abs,
        ...
        if asym {
            let mut col: Vec<f64> = (0..n)
                .filter(|&j| j != i)
                .map(|j| m[j * n + i] as f64)
                .collect();
            col.sort_by(|a, b| a.partial_cmp(b).unwrap());
            let col_mean: f64 = col.iter().sum::<f64>() / nn as f64;
            let rtopk = |k: usize| -> f64 {
                let kk = k.min(nn);
                r[nn - kk..].iter().map(|x| x.0 as f64).sum::<f64>() / kk as f64
            };
            let ctopk = |k: usize| -> f64 {
                let kk = k.min(nn);
                col[nn - kk..].iter().sum::<f64>() / kk as f64
            };
            feats.extend([
                mean,
                col_mean,
...
fn graph_features(m: &[f32], n: usize, prefix: &str) -> Vec<(String, Vec<f32>)> {
    ...
    for i in 0..n {
        ...
        let nb: Vec<usize> = (row_ptr[i] as usize..row_ptr[i + 1] as usize)
            .map(|k| col_idx[k] as usize)
            .collect();
...
fn extract_factors(
    ...
    for (nm, _) in combos.iter() {
        order.push((nm.clone(), false));
    }
    ...
    for (nm, vecs) in spectral_mats.iter() {
        for j in 0..SPECTRAL_K {
            add(
                &format!("spec_{nm}__ev{}", j + 1),
                vecs[j * n..(j + 1) * n].to_vec(),
            );
        }
    }
    ...
    for (li, &lag) in [60usize, 300, 900].iter().enumerate() {
        ...
        for (i, st) in stocks.iter().enumerate() {
            ...
            let va: f64 = a
                .iter()
                .map(|&v| (v as f64 - ma) * (v as f64 - ma))
                .sum::<f64>()
                / a.len() as f64;
            let vb: f64 = b
                .iter()
                .map(|&v| (v as f64 - mb) * (v as f64 - mb))
                .sum::<f64>()
                / b.len() as f64;
            let den = (va * vb).sqrt();
            let num: f64 = a
                .iter()
                .zip(b.iter())
                .map(|(&u, &v)| (u as f64 - ma) * (v as f64 - mb))
                .sum::<f64>()
                / a.len() as f64;
    ...
    // ret60 std/skew/kurt; 活跃度剖面; 基础统计
    {
        ...
        for (i, st) in stocks.iter().enumerate() {
            ...
            c_std[i] = sd as f32;
            c_skew[i] = if sd > 1e-12 {
                (m3 / sd.powi(3)) as f32
            } else {
                0.0
            };
            c_kurt[i] = if sd > 1e-12 {
                (m4 / sd.powi(4) - 3.0) as f32
            } else {
                0.0
            };
    ...
    }
    {
        ...
        for (i, st) in stocks.iter().enumerate() {
            ...
            cols[7].push(
                st.s60.iter().map(|&v| (v > 0.0) as i32 as f32).sum::<f32>() / SCALE_60 as f32,
            );
        }
        self_names.extend([
            "log_n",
