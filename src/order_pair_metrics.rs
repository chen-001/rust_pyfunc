...
pub fn calculate_order_pair_metrics(
    ...
    // 计算日均价格
    let today_avg_price: f64 =
        today_sorted.prices.iter().sum::<f64>() / today_sorted.prices.len() as f64;
    let yest_avg_price: f64 =
        yest_sorted.prices.iter().sum::<f64>() / yest_sorted.prices.len() as f64;

    // 计算全天价格极差
    let today_price_min = today_sorted
        .prices
        .iter()
        .fold(f64::INFINITY, |a, &b| a.min(b));
    let today_price_max = today_sorted
        .prices
        .iter()
        .fold(f64::NEG_INFINITY, |a, &b| a.max(b));
    let today_price_range = today_price_max - today_price_min;
    let yest_price_min = yest_sorted
        .prices
        .iter()
        .fold(f64::INFINITY, |a, &b| a.min(b));
    let yest_price_max = yest_sorted
        .prices
        .iter()
        .fold(f64::NEG_INFINITY, |a, &b| a.max(b));
    ...
    // 分离买单和卖单（排序确保确定性：HashMap 的迭代顺序每个实例都不同）
    ...
    // 合并所有配对
    let all_pairs: Vec<(i64, i64)> = buy_pairs
        .into_iter()
...
fn compute_shadow_orders(orders: &HashMap<i64, OrderInfo>) -> HashMap<i64, ShadowSet> {
    ...
    for i in 0..n {
        ...
        shadows.insert(order.order_id, ShadowSet::from_unsorted(shadow_ids));
    }
...
fn compute_metrics(
    ...
    today_shadow: &HashMap<i64, ShadowSet>,
    yest_shadow: &HashMap<i64, ShadowSet>,
) -> [f64; 16] {
    ...
    // 3. 带方向的订单体量差
    let yest_signed = if yest_order.is_buy {
        yest_order.total_turnover
    } else {
        -yest_order.total_turnover
    };
    let today_signed = if today_order.is_buy {
        today_order.total_turnover
    } else {
        -today_order.total_turnover
    };
    ...
    // 9. 成交时长
    metrics[8] = (yest_order.last_time - yest_order.first_time)
        - (today_order.last_time - today_order.first_time);

    // 10. 影子订单重叠度
    metrics[9] = calculate_shadow_overlap(
        today_order.order_id,
        yest_order.order_id,
        today_shadow,
        yest_shadow,
    );
...
fn calculate_shadow_overlap(
    ...
    let intersection = today_neighbors.intersection_count(yest_neighbors);
    let union = today_neighbors.union_count(yest_neighbors);
...
fn calculate_lz_complexity_market(
    sorted: &SortedTrades,
    order: &OrderInfo,
    window_size: usize,
) -> f64 {
...
fn calculate_fractal_dimension_market(
    sorted: &SortedTrades,
    order: &OrderInfo,
    window_size: usize,
) -> f64 {
...
/// 4. 计算捕食压力 - 基于市场全局数据
/// 该订单成交前N笔市场中主动买/卖的比例
fn calculate_predation_pressure_market(
    sorted: &SortedTrades,
    order: &OrderInfo,
    window_size: usize,
) -> f64 {
...
/// 该订单成交前N笔市场成交的时间间隔CV
fn calculate_interval_cv_market(
    sorted: &SortedTrades,
    order: &OrderInfo,
    window_size: usize,
) -> f64 {
    ...
    // 计算时间间隔
    let intervals: Vec<f64> = (1..n).map(|i| times[i] - times[i - 1]).collect();
...
pub fn calculate_order_pair_metrics_more_v2_faster(
    ...
    let mut yest_market_metrics: HashMap<i64, [f64; 5]> = HashMap::with_capacity(yest_orders.len());
