...
fn compute_regression_operator(
    style_rows: &[f64],
    n_rows: usize,
    n_features: usize,
) -> Option<DMatrix<f64>> {
...
fn neutralize_impl<T: FloatLike, O: OutputLike>(
    ...
    if style.ndim() != 3 || factor.ndim() != 3 {
        return Err(PyValueError::new_err(
            "style_cube 和 factor_block 都必须是三维数组",
        ));
    }
...
fn legacy_neutralize_impl<T: FloatLike, O: OutputLike>(
    ...
    let worker = |(date_idx, day_output): (usize, &mut [O])| {
        ...
        let Some(regression_operator) = compute_regression_operator(
            &style_valid_rows,
            style_valid_stock_indices.len(),
            n_features,
        ) else {
            return;
        };
            ...
            for (row_idx, &style_pos) in valid_style_positions.iter().enumerate() {
                ...
                for feature_idx in 0..n_features {
                    beta_values[feature_idx] +=
                        regression_operator[(feature_idx, style_pos)] * y_value;
                }
            }
...
pub fn tail_v2_neutralize_block<'py>(
    ...
    let output = py.allow_threads(|| {
        neutralize_impl::<f64, f64>(style, factor, rank_before, min_valid, true)
    })?;
...
pub fn tail_v2_neutralize_block_f32<'py>(
    ...
    let factor = factor_block.as_array();
    let output = py.allow_threads(|| {
        neutralize_impl::<f32, f64>(style, factor, rank_before, min_valid, true)
    })?;
    Ok(output.into_pyarray(py).to_owned())
}
...
pub fn tail_v2_neutralize_block_f32_out<'py>(
    ...
    let factor = factor_block.as_array();
    let output = py.allow_threads(|| {
        neutralize_impl::<f32, f32>(style, factor, rank_before, min_valid, true)
    })?;
    Ok(output.into_pyarray(py).to_owned())
}
...
pub fn tail_v4_neutralize_block_f32_out_legacy<'py>(
    ...
    let factor = factor_block.as_array();
    let output = py.allow_threads(|| {
        legacy_neutralize_impl::<f32, f32>(style, factor, rank_before, min_valid, true)
    })?;
    Ok(output.into_pyarray(py).to_owned())
}
...
pub fn tail_v4_neutralize_block_f64_out_legacy<'py>(
    ...
    let factor = factor_block.as_array();
    let output = py.allow_threads(|| {
        legacy_neutralize_impl::<f64, f64>(style, factor, rank_before, min_valid, true)
    })?;
    Ok(output.into_pyarray(py).to_owned())
}
