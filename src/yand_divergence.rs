...
fn slice_theta_3(
    ...
    for pi in 0..3 {
        ...
        let scx = sx / sn as f64 - zx;
        let scy = sy / sn as f64 - zy;
...
#[pyfunction]
pub fn debug_compare_thetas(price: Vec<f64>, volume: Vec<f64>) -> Vec<f64> {
    if volume.is_empty() {
        return vec![];
    }
    let mut dp = Vec::new();
    ...
    let mut dp_f = Vec::new();
    let mut vol_f = Vec::new();
    for i in 0..dp.len() {
        if dp[i].abs() < dp99 && vol[i] < vol99 {
            dp_f.push(dp[i]);
            vol_f.push(vol[i]);
        }
    }
    ...
    for si in 0..5 {
        ...
        for &w in &windows {
            ...
            for j in 0..w {
                rx += data[j][0];
                ry += data[j][1];
                rxx += data[j][0] * data[j][0];
                rxy += data[j][0] * data[j][1];
                ryy += data[j][1] * data[j][1];
            }
            ...
            for i in 0..nd_w {
                ...
                let (cmx, cmy) = (x / wf - zx, y / wf - zy);
                let cgn = (cmx * cmx + cmy * cmy).sqrt();
                let c_dir = if cgn > 1e-10 {
                    Some((cmx / cgn, cmy / cgn))
                } else {
                    None
                };
                ...
                let p_dir = if vx.abs() > 1e-15 || vy.abs() > 1e-15 {
                    ...
                    let (evx, evy) = (cxy2, (vy - vx + d) / 2.0);
                    let evn = (evx * evx + evy * evy).sqrt();
                    if evn > 1e-10 {
                        let (mut pgx, mut pgy) = (evx / evn, evy / evn);
                        if pgx * (zx - mx2) + pgy * (zy - my2) < 0.0 {
                            pgx = -pgx;
                            pgy = -pgy;
                        }
                        Some((pgx, pgy))
                ...
                let s_dir = if two_d {
                    ...
                    if vx_s.abs() > 1e-15 {
                        let slope = (xy / wf - mx2 * my2) / vx_s;
                        let sn = (1.0 + slope * slope).sqrt();
                        let (mut sgx, mut sgy) = (1.0 / sn, slope / sn);
                        if sgx * (zx - mx2) + sgy * (zy - my2) < 0.0 {
                            sgx = -sgx;
                            sgy = -sgy;
                        }
                        Some((sgx, sgy))
                ...
                for gi in 0..ng {
                    let Some((gx, gy)) = dirs[gi] else {
                        continue;
                    };
                    // 新方法
                    let new_out = slice_theta_3(data, i, w, &pcts, zx, zy, gx, gy);
                    for pi in 0..3 {
                        if let Some(th) = new_out[pi] {
                            tn[gi][pi].push(th);
                        }
                    }
                    // 旧方法
                ...
                if i + 1 < nd_w {
                    x += data[i + w][0] - data[i][0];
                    y += data[i + w][1] - data[i][1];
...
pub fn debug_theta(
    ...
    if volume.is_empty() || window > 1024 {
        return vec![];
    }
    let mut dp = Vec::new();
    let mut vol = Vec::new();
...
pub fn compute_divergence(
    ...
    if volume.is_empty() || windows.iter().any(|&w| w > 1024) {
        return vec![];
    }
    // 价格变化段聚合
    let mut dp = Vec::new();
    ...
    for si in 0..5 {
        ...
        for &w in &windows {
            ...
            for pi in 0..3 {
                for gi in 0..ng {
                    result.extend(compute_5stats(&theta[gi][pi]));
                }
            }
        }
    }
