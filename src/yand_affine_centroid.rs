...
pub fn compute_affine_centroid(
    bid_prices: Vec<Vec<f64>>,
    bid_vols: Vec<Vec<f64>>,
    ask_prices: Vec<Vec<f64>>,
    ask_vols: Vec<Vec<f64>>,
) -> Vec<Vec<f64>> {
    ...
    for i in 0..n {
        ...
        // 有效数据
        let mut bid_p_valid = Vec::new();
        let mut bid_v_valid = Vec::new();
        let mut ask_p_valid = Vec::new();
        let mut ask_v_valid = Vec::new();
        for j in 0..10 {
            if j < bp.len() && bp[j] > 0.0 && bv[j] > 0.0 {
                bid_p_valid.push(bp[j]);
                bid_v_valid.push(bv[j]);
            }
            if j < ap.len() && ap[j] > 0.0 && av[j] > 0.0 {
                ask_p_valid.push(ap[j]);
                ask_v_valid.push(av[j]);
            }
        }
        ...
        let bid_cen = bid_p_valid
            .iter()
            .zip(bid_v_valid.iter())
            .map(|(p, v)| p * v)
            .sum::<f64>()
            / sum_bv;
        let ask_cen = ask_p_valid
            .iter()
            .zip(ask_v_valid.iter())
            .map(|(p, v)| p * v)
            .sum::<f64>()
            / sum_av;
        ...
        for (idx, delta_tick) in [1, 3, 5, 10].iter().enumerate() {
            ...
            if bid_inside + ask_inside > 0.0 {
                let bid_avg = if bid_inside > 0.0 {
                    bid_wsum / bid_inside
                } else {
                    mid
                };
                let ask_avg = if ask_inside > 0.0 {
                    ask_wsum / ask_inside
                } else {
                    mid
                };
        ...
        for j in 0..6 {
            categories[6 + j].push(level_imbs[j]);
        }
        categories[12].push(spread);
        categories[13].push(total_depth);
        for j in 0..4 {
            categories[14 + j].push(sc_vals[j]);
        }
    }
    categories
}
...
pub fn compute_ts_stats(series: Vec<f64>) -> Vec<f64> {
    ...
    let early_mean = if early.is_empty() {
        f64::NAN
    } else {
        early.iter().sum::<f64>() / early.len() as f64
    };
    let early_std = if early.len() > 1 {
        (early.iter().map(|v| (v - early_mean).powi(2)).sum::<f64>() / early.len() as f64).sqrt()
    } else {
        f64::NAN
    };
    ...
    let late_std = if late.len() > 1 {
        (late.iter().map(|v| (v - late_mean).powi(2)).sum::<f64>() / late.len() as f64).sqrt()
    } else {
        f64::NAN
    };
