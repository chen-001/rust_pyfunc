...
fn run_single_worker_manager(
    ...
    'outer: loop {
        ...
        // Level2 数据目录透传：worker 子进程按同一目录读取原始数据。
...
pub fn run_factor_pipeline_minute(
    ...
    // 分钟数据目录覆盖：进程内生效到本函数结束；worker 子进程通过环境变量透传。
    let _data_root_guard = crate::data_paths::set_minute_root(data_root.as_deref());
    println!("📂 分钟数据目录: {}", crate::data_paths::describe_minute());
...
fn run_single_minute_worker(
    ...
    'outer: loop {
        ...
        cmd.env("OPENBLAS_NUM_THREADS", "1");
        cmd.env("MKL_NUM_THREADS", "1");
        // 分钟数据目录透传（pipeline 参数 data_root）
        if let Some(root) = crate::data_paths::override_minute() {
            cmd.env(crate::data_paths::ENV_MINUTE_ROOT, root);
        }
        if let Some(idx) = core_affinity_idx {
            cmd.env("RUST_PYFUNC_CORE_AFFINITY_IDX", idx.to_string());
        }
...
pub fn pipeline_hot_stock_pool_v2(date: i64, expected_len: usize) -> Vec<TaskResult> {
    if expected_len != crate::hot_stock_pool_metrics::N_FACTORS {
        eprintln!(
            "hot_stock_pool_v2 expected_len错误 [{date}]: {expected_len} != {}",
            crate::hot_stock_pool_metrics::N_FACTORS
        );
        return Vec::new();
    }
    match crate::hot_stock_pool_metrics::compute_hot_stock_pool_v2_full(date) {
        Ok((codes, vals)) => vals
            .chunks(expected_len)
            .zip(codes.iter())
            .map(|(facs, code)| TaskResult {
            ...
            .collect(),
        Err(e) => {
            eprintln!("hot_stock_pool_v2 error [{date}]: {e:?}");
            Vec::new()
        }
    }
}
