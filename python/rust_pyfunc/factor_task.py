...
# 例外：hm 系列因子约定命名——hm+数字（如 hm100、hm89）、hm+数字+_ind（如 hm100_ind）、
#      hm+数字+_ssm（如 hm100_ssm，双边单调度 SSM 版本）、hm+数字+_ssm+序号（如 hm100_ssm2）。
...
def cmd_submit(args):
    ...
    if name_err:
        ...
        print(
            ...
            "hm+数字（如 hm100）、hm+数字_ind（如 hm100_ind）、"
            "hm+数字_ssm（如 hm100_ssm）亦可",
            file=sys.stderr,
        )
        sys.exit(1)
